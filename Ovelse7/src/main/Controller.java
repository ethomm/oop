...
public class Controller {
	...
	public Controller(Gui gui, KundeOversikt kundeoversikt, LeverandorOversikt leverandoroversikt, SalgsOversikt salgsoversikt, VareLager varelager) {
		...
		this.gui.addActionTilListeVarelager(new visVareDetaljerIOversikt());
...
//Metoder for å håndtere data
	...
	private void hentVareListeOversikt() {
		...
		int pattedyr = 0;
		int reptiler = 0;
		int andreprodukter = 0;
		int fisk = 0;
		
		//Finner ut hvor mange av hvert produkt som finnes
		for (Produkt prod : VareLager.produktListe){
			if(prod instanceof Pattedyr){
				pattedyr ++;
			}else if(prod instanceof Reptil){
				reptiler++;
			}else if(prod instanceof Fisk){
				//Caster for å få tilgang til funksjonen getAntAv....
				if (((Fisk)prod).getAntAvTypeAnnetProdukt()>=0){
				 fisk +=((Fisk) prod).getAntAvTypeAnnetProdukt();	
				}
			}else if(prod instanceof AnnetProdukt){
				//Gjør det samme her
				if(((AnnetProdukt) prod).getAntAvTypeAnnetProdukt() >=0){
					andreprodukter+= ((AnnetProdukt) prod).getAntAvTypeAnnetProdukt();
				}
			}
		}
		
		//Legger sammen totalen
		int total = pattedyr+reptiler+andreprodukter+fisk;
		
		//Skriver ut til de tomme tekstfeltene 
		gui.printAntallPattedyr.setText(Integer.toString(pattedyr));
		gui.printAntallFisk.setText(Integer.toString(fisk));
		gui.printAntallAndreProdukter.setText(Integer.toString(andreprodukter));
		gui.printAntallReptiler.setText(Integer.toString(reptiler));
		gui.printAntallTotalt.setText(Integer.toString(total));
	...
	//Viser all informasjon om vare som er valgt i VareLager Oversikten
	private class visVareDetaljerIOversikt implements ListSelectionListener{
			@Override
